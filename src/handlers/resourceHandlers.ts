...
export async function handleListResources() {
  try {
    ...
    let resourceBaseUrl: URL;      // Create appropriate URL based on database type
    if (dbType === 'sqlserver' && dbInfo.server && dbInfo.database) {
      resourceBaseUrl = new URL(`sqlserver://${dbInfo.server}/${dbInfo.database}`);
    } else if (dbType === 'postgresql' || dbType === 'mysql') {
      // For PostgreSQL and MySQL use host property
      const hostname = (dbInfo as any).host || 'localhost';
      const db = dbInfo.database || '';
      resourceBaseUrl = new URL(`${dbType}://${hostname}/${db}`);
    } else {
